...
You can cancel a persistent upgrade from the admin or the API right up
until the device starts writing the new image; once the flash is underway,
...
due and launches them. Everything is re-checked at that point — devices,
permissions and firmware — because plenty can change between scheduling a
rollout and running it.
